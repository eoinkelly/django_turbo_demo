...
def install(ctx, skip_install_playwright: bool = False):
    """
    Install system dependencies necessary for the turbo_demo project.
...
def typing(ctx):
    ...
    with ctx.prefix(f"export PYTHONPATH=$:{src_path}"):
        try:
...
def build_docs(ctx):
    """
    Build the turbo_demo documentation
    """
...
def run_docs(ctx):
    """
    Run the turbo_demo documentation locally
    """
